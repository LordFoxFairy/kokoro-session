...
const sessionEventSchema = z.discriminatedUnion("event", [
  ...
  z.object({ event: z.literal("tool.returned"), ...envelopeFields, payload: z.object({ segment_id: z.string().min(1), tool_id: z.string().min(1), name: z.string().min(1), result: z.string(), is_error: z.boolean(), rejected: z.boolean().optional() }).strict() }).strict(),
  ...
  z.object({ event: z.literal("subagent.finished"), ...envelopeFields, payload: z.object({ segment_id: z.string().min(1), subagent_id: z.string().min(1), name: z.string().min(1), subagent_type: z.string().min(1), source: z.enum(["built-in", "config-custom", "runtime-custom"]) }).strict() }).strict(),
